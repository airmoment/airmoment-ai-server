"""build_quantile_dataset_v2.py

build_quantile_dataset.py + 외부 feature(유가, 환율) 추가 버전.
...
출력: data_0526/processed/quantile_dataset_v2.csv
"""
...
RAW_DIR      = PROJECT_ROOT / 'data_0526' / 'raw'
OUTPUT_DIR   = PROJECT_ROOT / 'data_0526' / 'processed'
...
# ---------------------------------------------------------------------------
# Stage 2 — History summary features
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 4 (NEW) — External features: 유가 + 환율
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 5 — Multi-horizon future price targets
# ---------------------------------------------------------------------------

def extract_horizon_targets(summary: pd.DataFrame, history: pd.DataFrame) -> pd.DataFrame:
    """각 search event에 대해 +1/+3/+7/+14일의 실제 가격을 찾는다.

    trajectory 단위 history 사용 (같은 노선+출발일의 모든 검색).
    look-ahead 방지: searched_at 이후의 관측값만 사용.
    target_ts ± TOLERANCE_SEC 이내의 가장 가까운 관측값 사용.
    """
    hist = history.copy()
    hist = hist.rename(columns={'price': 'history_price'})
...
def main() -> None:
    ...
    for col in EXT_COLS:
        if col in df.columns:
            valid = df[col].notna().sum()
            print(f'  {col:<20}: {valid}/{len(df)} ({valid/len(df)*100:.1f}%)')

    out_path = OUTPUT_DIR / 'quantile_dataset_v2.csv'
